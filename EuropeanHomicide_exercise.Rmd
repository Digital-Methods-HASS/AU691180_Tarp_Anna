...
- How would you characterize the trend compared to the homicides above?
 #We can see that the rulers reign longer, and that is maybe because they live longer. They usually stop ruling when they die. The homicides are falling in contrast to the Danish rulers. 

```{r eval = TRUE}

kings_1 <- read_csv2("data/kings_1.csv")
kings_1 %>% 
  mutate(Midyear=End_date-(End_date-Start_date)/2) %>% 
  ggplot(aes(x = Midyear, y = Duration)) +
  geom_smooth() + 
  labs(title = "Reign", x = "Reign Midyear", y = "Reign Duration")
